...
export class DiscordBotService {
  ...
  private async handleCommand(message: Message) {
    ...
    await this.logActivity('CMD', `User @${message.author.username} executed command: !relay ${command}`);
    ...
    switch (command) {
      ...
      case 'test':
        ...
        break;
      default:
        await message.reply('Unknown command. Available commands: status, add, remove, test');
    }
  }
  ...
  private async handleStatusCommand(message: Message) {
    ...
    const embed = {
      ...
      color: 0x5865F2,
      fields: [
        { name: 'Status', value: this.isConnected ? '🟢 Online' : '🔴 Offline', inline: true },
        { name: 'Uptime', value: uptime, inline: true },
        { name: 'Active Relays', value: activeRelays.toString(), inline: true },
        { name: 'Messages Relayed', value: stats?.messagesRelayed?.toString() || '0', inline: true },
        { name: 'API Calls', value: stats?.apiCalls?.toString() || '0', inline: true },
      ],
    };
  ...
  private async handleAddRelayCommand(message: Message, args: string[]) {
    if (args.length < 2) {
      await message.reply('Usage: !relay add <source_channel_id> <target_channel_id> [bidirectional]');
      return;
    }
  ...
  private async handleTestCommand(message: Message) {
    ...
    for (const relay of activeRelays) {
      try {
        const targetChannel = await this.client.channels.fetch(relay.targetChannelId) as TextChannel;
  ...
  private async handleMessageRelay(message: Message) {
    ...
    for (const relay of activeRelays) {
      ...
      if (shouldRelay) {
        const targetChannelId = message.channelId === relay.sourceChannelId 
        ...
        try {
          ...
          await this.logActivity('RELAY', `Message relayed from #${(message.channel as TextChannel).name} to #${targetChannel.name} (ID: ${message.id})`);
        } catch (error) {
          console.error(`Failed to relay message to ${targetChannelId}:`, error);
          await this.logActivity('ERROR', `Failed to relay message to channel ${targetChannelId}: ${error}`);
        }
      }
    }
  }

  private async logActivity(type: string, message: string, channelId?: string, userId?: string) {
